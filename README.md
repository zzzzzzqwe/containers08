...
Первые 6 коммитов тесты не выполнялись из-за ошибок связанных с:
- отсутствием конфигурации `main.yml`
- неверными названиями переменных
- неверными путями к файлам
- устаревшей версией `PHP` (в `Dockerfile` сменил версию `PHP` c 7.4 на 8.2 для поддержки `str_contains()`)

После нескольких исправлений тесты стали успешно прогоняться:
...
`Непрерывная интеграция (Continuous Integration, CI)` — это практика в разработке ПО, при которой изменения в коде автоматически проверяются и тестируются каждый раз, когда они попадают в репозиторий (например, через push или pull request). Она нужна чтобы быстро выявлять ошибки, интегрировать изменения как можно чаще и автоматизировать проверку качества кода.

2. Для чего нужны юнит-тесты? Как часто их нужно запускать?

`Юнит-тесты` — это автоматические тесты, проверяющие работу отдельных функций или методов программы.

Зачем они нужны:
- Для обнаружения ошибок на ранней стадии;
- Для уверенности в том, что изменения в коде не сломали старый функционал (регрессии);
- Для ускорения разработки и рефакторинга.

Их нужно запускать при каждом измении кода, автоматически при каждом push и pull request (в рамках CI) и локально перед коммитом (по возможности).

3. Что нужно изменить в `.github/workflows/main.yml`, чтобы тесты запускались при каждом создании запроса на слияние (Pull Request)?

Необходимо добавить  блок `pull_request:` в секцию `on:`:
```yml
on:
  pull_request:
    branches:
...
Нужно добавить следующее после `docker rm`:
```yml
...
## Вывод
В рамках выполненной работы было создано `PHP`-приложение с использованием базы данных `SQLite` и реализована система автоматического тестирования через `GitHub Actions`. Для контроля качества кода были написаны юнит-тесты для всех методов классов `Database` и `Page`. Эти тесты проверяют корректность выполнения базовых операций с базой данных, работу шаблонов и возвращаемый `HTML`. Тесты запускаются автоматически в `CI` при каждом обновлении ветки main или создании pull request.
